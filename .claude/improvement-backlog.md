...
> 이 문서는 코드 감사(2026-05-12) + 시지푸스 루프(2026-05-12) 결과를 반영합니다.
> ✅ = 코드 확인 완료 / ⚠️ = 부분 구현 / ❌ = 미구현

## 시지푸스 루프 (2026-05-12) 완료 항목

| 항목 | 수정 내용 | 커밋 |
|------|-----------|------|
| **상단 UI 배치** | TopNavBar: `md:left-64`를 `hasSidebar` prop 조건부 적용 (사이드바 없는 페이지 256px 공백 버그 수정), 브랜드명 비사이드바 페이지서 항상 표시, 탭 활성 상태 버그(검색페이지에서 '단일 기업 보고서' 탭 잘못 활성화) 수정 | bd2ccc5 |
| **재무 가독성 KRW** | formatKRW: 백만원→조원/억원/백만원 변환 (예: "302,231,360"→"302.2조원"), 최신 연도 강조, 성장률 ▲▼ 색상 배지 | bd2ccc5 |
| **재무 가독성 FMP** | formatFMPValue: USD→B/M/K 변환 (예: "394,328,000,000"→"394.3B"), 단위 라벨 명확화 | 17c7484 |
| **출처 배지 범위** | displayHelpers: h1~h6, blockquote에 processChildren 적용 (heading/인용문 내 [DART-1] 등 배지 변환) | bfad2fc |
| **blockquote 스타일** | bg-primary/5 + rounded-r-lg 추가 | bfad2fc |
| **KST 타임존** | generatedAt 날짜 표시에 `timeZone: 'Asia/Seoul'` 명시 | bfad2fc |
