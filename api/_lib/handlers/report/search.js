...
export default async function handler(req, res) {
  const logger = createLogger('/api/report/search');
  ...
  try {
    ...
    // 4. 오케스트레이터 기동 (Stage 1 Search Only)
    ...
    logger.error('Generate report search stage failed', { companyName, error: err.message, stack: err.stack, code: err.code });
