...
export default async function handler(req, res) {
  if (req.method !== 'POST') {
    return res.status(405).json(createErrorResponse(ErrorCategory.VALIDATION, 'METHOD_NOT_ALLOWED', '허용되지 않는 방식입니다.'));
  }

  try {
    const { email, password } = req.body;
    if (!email || !password) {
      return res.status(400).json(createErrorResponse(ErrorCategory.VALIDATION, 'MISSING_FIELDS', '이메일과 비밀번호를 입력해주세요.'));
    }

    const userRecord = await findUserByEmail(email);
    if (!userRecord) {
      return res.status(401).json(createErrorResponse(ErrorCategory.AUTH, 'INVALID_CREDENTIALS', '이메일 혹은 비밀번호가 일치하지 않습니다.'));
    }
